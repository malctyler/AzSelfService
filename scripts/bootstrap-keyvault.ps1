param(
    ...
    [string]$SoftwareStorageAccountName = 'software',
    [string]$SoftwareContainerName = 'packages',
...
$softwareStorageExists = az storage account list `
    --resource-group $ResourceGroupName `
    --query "[?name=='$SoftwareStorageAccountName'] | length(@)" `
    -o tsv

if ($softwareStorageExists -eq '0') {
    Write-Step "Creating software storage account $SoftwareStorageAccountName"
    az storage account create `
    ...
    Write-Step "Retrofitting software storage account $SoftwareStorageAccountName"
...
$softwareStorageId = az storage account show `
    --name $SoftwareStorageAccountName `
    --resource-group $ResourceGroupName `
    --query id `
    -o tsv

Write-Step "Assigning Storage Blob Data Owner role for software storage account"
Invoke-AzRoleAssignmentWithRetry `
    -PrincipalAppId $ServicePrincipalAppId `
    -Scope $softwareStorageId `
    -Role 'Storage Blob Data Owner'

Write-Step "Creating blob container $SoftwareContainerName in $SoftwareStorageAccountName"
for ($attempt = 1; $attempt -le 10; $attempt++) {
    try {
        az storage container create `
            --name $SoftwareContainerName `
            --account-name $SoftwareStorageAccountName `
            --auth-mode login `
            --only-show-errors | Out-Null
        Write-Host "Container '$SoftwareContainerName' created." -ForegroundColor Green
        break
    }
    catch {
        if ($attempt -eq 10) {
...
Write-Host "Software account   : $SoftwareStorageAccountName"
Write-Host "Software container : $SoftwareContainerName"
